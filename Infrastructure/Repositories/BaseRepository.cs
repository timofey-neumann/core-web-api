...
public class BaseRepository<T> : IBaseRepository<T> where T : class
{
    ...
    {
        var data = await _dbContext.Set<T>().FindAsync(id, cancellationToken) ?? throw new NotFoundException("No data found");
    ...
    public virtual async Task<T> GetById<Tid>(List<Expression<Func<T, object>>> includeExpressions, Tid id, CancellationToken cancellationToken = default)
    {
        ...
        var data = await query.SingleOrDefaultAsync(x => EF.Property<Tid>(x, "Id").Equals(id), cancellationToken) ?? throw new NotFoundException("No data found");
